...
Do not spend GPU time blindly scaling this checkpoint yet. First pull fresh
closed-loop relabel CSVs generated by:
...
Then train on `trace_augmented_rejoin.csv` plus those relabel CSVs, using
`--no-prev-command-features --no-sequence-features` for the deployable feature
policy. The first local relabel iteration fit offline but still failed the
simulator. The second `rejoin` relabel iteration fit the closed-loop failure
labels (`mse=0.00080114`) and kept held-out `s_curve` close
(`mse=0.00213601`), but the 10 s easy rollout still ended `DNF gates=0/3`.
Command-source logging on the latest 10 s failure showed
`reactive_fallback=528` rows and `learned=131` rows, so spend T4 time on
