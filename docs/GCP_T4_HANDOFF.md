...
Then train on `trace_augmented.csv` plus those relabel CSVs. The first local
iteration fit the relabel data offline but still failed the simulator, so the
next useful GPU run should happen only after improving the yaw/altitude recovery
teacher.
